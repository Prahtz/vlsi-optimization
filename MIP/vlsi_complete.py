...
class VLSIComplete(VLSI):
    ...
    def define_model(self):
        ...
        cumulative_x = cumulative(x, [m[i][0] for i in range(n_blocks)], [m[i][1] for i in range(n_blocks)], h, 'x')
        cumulative_y = cumulative(y, [m[i][1] for i in range(n_blocks)], [m[i][0] for i in range(n_blocks)], w, 'y')

        constraints = obj + y_bounds + no_overlap + cumulative_x + cumulative_y 
